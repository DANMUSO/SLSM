...
<div class="card card-primary card-outline">
              ...
              <div class="card-body">
                <div class="tab-content">
                  <div class="active tab-pane" id="activity">
                    <!-- Post -->
                    <div class="post">
                    <center> <label>List of inquiriess</label></center>
                    <div class="row"><div class="col-md-6"></div>
        <div class="col-md-4"></div>
    <div class="col-md-2"><a href="{{ route('admin.inquiries.create')}}" class="btn btn-primary">Add inquiry</a></div></div>
                    <div class="container">
    ...
    <tbody id="myTable">
      ...
      <td>{{$inquiry ->phone}}</td> 
      <td>{{$inquiry ->email}}</td>
      <td>{{$inquiry ->person}}</td>   
      <td>{{$inquiry ->purpose}}</td> 
      <td>{{$inquiry ->atime}}</td> 
      <td>{{$inquiry ->adate}}</td>
      <td>
                     <a href="{{ route('admin.inquiries.edit', $inquiry ->id)}}" class="btn btn-primary fa fa-edit" >
                     </a>
                     <form id="delete_from_{{$inquiry->id}}" method="POST" action="{{ route('admin.inquiries.destroy', $inquiry->id) }}">
                      {{ csrf_field() }}
                      {{ method_field('DELETE') }}

                      <div class="form-group">
                          <a href="javascript:void(0);" data-id="{{$inquiry->id}}" class="_delete_data">
